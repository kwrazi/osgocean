...
// ---------------------------------------------------------------------
// create a sequence of integers i such that,
//     m_start <= i < m_end
// with stride = m_step
class StepPattern {
public:
  StepPattern(const int start, const int end, const int step = 1)
    : m_start(start)
    , m_end(end)
    , m_step(step)
  {
    assert (m_step != 0);
    assert ((m_step > 0) ? (m_start < m_end) : (m_end < m_start));
  }
  bool contains (const int num) {
    if ((num >= m_start) && (num < m_end)) {
      if (((num - m_start) % m_step) == 0)
        return true;
    }
    return false;
  }
  ...
  friend bool operator > (const int &lhs, const StepPattern &rhs) {
    return rhs < lhs;
  }
  friend bool operator < (const int &lhs, const StepPattern &rhs) {
    return rhs > lhs;
...
// based on:
//   https://gist.github.com/jasonbeverage/936c644916ebcee0d6c4

class CaptureFrame : public osgViewer::ScreenCaptureHandler::CaptureOperation {
public:
  CaptureFrame(const std::string& filename)
  ...
  {
    std::stringstream ss;
    ss << m_filePrefix << "-" << std::setfill('0') << std::setw(5) << m_frameCount << ".png";
    osg::notify(osg::NOTICE) << "Save image '" << ss.str() << "' (context_id="
                             << context_id << ")" << std::endl;
...
int main(int argc, char *argv[])
{
    ...
    arguments.getApplicationUsage()->addCommandLineOption("--reflectionDamping <factor>", "reflection damping factor. Default: 0.35");
    arguments.getApplicationUsage()->addCommandLineOption("--windScale <factor>", "wind scale multiplier. Default: 1e-8");
    ...
    arguments.getApplicationUsage()->addCommandLineOption("--vbo","use VBO.");
    arguments.getApplicationUsage()->addCommandLineOption("--compositeViewer","enable composite viewer.");
    arguments.getApplicationUsage()->addCommandLineOption("--firstViewLast","swap view when in composite viewer mode.");
    arguments.getApplicationUsage()->addCommandLineOption("--disableEffectsForSecondView","disable effects for second view when in composite viewer mode.");
    arguments.getApplicationUsage()->addCommandLineOption("--useShadows","enable shadow effects.");
    arguments.getApplicationUsage()->addCommandLineOption("--debugDraw","enable debug mod for draw pipeline.");
    arguments.getApplicationUsage()->addCommandLineOption("--initialCameraPosition <x>,<y>,<z>","set the comera intial position at (default: 0,0,20).");
    arguments.getApplicationUsage()->addCommandLineOption("--outputImage", "Specify output image files prefix name.");
    arguments.getApplicationUsage()->addCommandLineOption("--totalFrames", "Specify the total number of sequential images to save (default: 0 = infinite)");
    ...
    while (arguments.read("--outputImage", outputImage));

    unsigned totalFrames = 0;
    while (arguments.read("--totalFrames", totalFrames));
    ...
    if (disableShaders)
    {
      osg::notify(osg::NOTICE) << "disabling shaders" << std::endl;
    ...
    if (compositeViewer)
    {
      osg::notify(osg::NOTICE) << "creating compositeviewer" <<  std::endl;
        ...
        if (firstViewLast)
        {
          osg::notify(osg::NOTICE) << "firstViewLast - swap views" <<  std::endl;
    ...
    {
      osg::notify(osg::NOTICE) << "creating single Viewer" <<  std::endl;
    ...
    if (!outputImage.empty()) {
      osg::notify(osg::NOTICE) << "Setting up capture to file." << std::endl;
      // setup screen capture handler
      osg::ref_ptr<osgViewer::ScreenCaptureHandler::CaptureOperation> writeFile =
        new CaptureFrame(outputImage);
      screenCaptureHandler = new osgViewer::ScreenCaptureHandler(writeFile.get());
      screenCaptureHandler->setKeyEventTakeScreenShot('a');
      screenCaptureHandler->setKeyEventToggleContinuousCapture('A');
      screenCaptureHandler->setFramesToCapture(0);

      // add the screen capture handler
      view->addEventHandler(screenCaptureHandler);
    }

    viewer->realize();

    // const auto camera_width = 800;
    // const auto camera_height = 768;

    // osgViewer::ViewerBase::Views views;
    // viewer->getViews(views);
    // osg::notify(osg::NOTICE) << "getViews found " << views.size() << " views" <<  std::endl;

    // osg::ref_ptr<osg::Image> img = new osg::Image;
    // img->allocateImage(camera_width, camera_height, 1, GL_RGBA, GL_UNSIGNED_BYTE);
    // views[0]->getCamera()->attach(osg::Camera::COLOR_BUFFER, img);

    bool captureMode = false;
    unsigned frameCount = 0;
    unsigned startFrame = 10;
    ...
    while(!viewer->done())
      {
        if (!outputImage.empty() && captureMode) {
          if (totalFrames > 0 && (frameCount - startFrame) > totalFrames) {
            captureMode = false;
            std::cout << "stopped @ frameCount=" << frameCount << std::endl;
            screenCaptureHandler->stopCapture();
            return 0;
          } else {
            std::cout << "capture @ frameCount=" << frameCount << std::endl;
            screenCaptureHandler->captureNextFrame(*viewer);
          }
        }
        // ----------------------------
